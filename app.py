...
co = cohere.Client('KVaVk5g8ERlIP02OZJ5tZ3Jz8HcA3nthd9eqxmNP') 
...
def mcq():
    if request.method == 'POST':
        try:
            # Get input text and number of questions from the form
            ...
            if not num_questions or not num_questions.isdigit() or int(num_questions) < 1:
                return jsonify({'error': 'Number of questions must be a valid positive integer'}), 400

            # Convert the number of questions to an integer
            num_questions = int(num_questions)

            # Generate questions in a batch (optimize the function to batch multiple requests)
            ...
            print(f"Error in /mcq route: {str(e)}")
